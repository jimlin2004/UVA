...
首先如果是偶數的塔，那麼答案一定是兩兩個塔的最大和(因為i可以直接用i - 2的禮物繼續看)
如果是奇數個塔的話，需要對答案二分搜
問題在於怎麼檢查m個禮物能不能分完
答案是神奇的greedy
...
int need[maxn];
int Left[maxn]; // 第i個塔放左邊的個數
int Right[maxn]; // 第i個塔放右邊的個數
int n;
...
bool check(int m)
{
    ...
    for (int i = 2; i <= n; ++i)
    {
        if (i % 2 == 0) // 偶數從左邊拿
        {
            ...
            Right[i] = need[i] - Left[i]; // 左邊放不下就放右邊
        }
        else // 奇數從右邊拿
        {
            Right[i] = min(need[i], R - Right[i - 1]);
            Left[i] = need[i] - Right[i];
        }
    }
