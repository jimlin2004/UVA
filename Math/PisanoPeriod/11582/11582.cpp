...
所以這題是在找出循環週期後，用快速冪並mod(循環週期長度)即可AC(快速冪記得mod)，
...
int main()
{
    ...
    //注意: 這裡沒算mod 1的循環長度，因為mod 1都等於0，沒有週期
    for (int i = 2; i < maxn; ++i)
    {
        ...
        {
            fib[i].emplace_back((fib[i][j - 1] % i + fib[i][j - 2] % i) % i);
    ...
    while (t--)
    {
        ...
        if (a == 0)
            puts("0");
        else if (n == 1) //當mod 1時任何數都是0，所以直接輸出0，避免我們沒算fib[1][i]的問題(Runtime error)
            puts("0");
        else
