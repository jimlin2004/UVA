...
bool bfs(int from, int to)
{
    ...
    while (!q.empty())
    {
        ...
        {
            if ((levelGraph[nextNode] == 0) && graph[currentNode][nextNode] > 0)
            {
...
int dfs(int from, int to, int bottleNeck)
{
    ...
    for (int nextNode = 1; nextNode <= nodeNum; ++nextNode)
    {
        if ((graph[from][nextNode] > 0) && (levelGraph[from] == levelGraph[nextNode] - 1) && canReachSink[nextNode])
        {
