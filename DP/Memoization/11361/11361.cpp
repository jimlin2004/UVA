...
/*
    ...
    此時輸入 a, b, k 答案即為 sumf(b + 1) - sumf(a) <- 前綴和的概念
    ...
    利用上面的23***例子，可以看出遞迴式
    dfs(d, m1, m2) = sum{ f(d - 1, (m1 - x) mod k, (m2 - x * 10 ^ (d - 1))) mod k | x = 0 ~ 9 }
    ...
    2 ^ 31 = 2147483648
    最多10位數，數字和最多 2 + 9 * 9 = 83(估算九位數九個九)
    只要 k > 83 就不可能存在[a, b]之間數字和 mod k = 0
    所以一定是0
